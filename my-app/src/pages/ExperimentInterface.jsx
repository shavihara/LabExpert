...
function ExperimentInterface() {
  ...
  const dataPointsRef = useRef([]); // Add ref to track data points
  ...
  useEffect(() => {
    const connectSSE = () => {
      ...
      eventSource.onmessage = (event) => {
        ...
        // Use functional update with ref to ensure we always have latest data
        const newDataPoint = {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      if (elapsedTime === 0) {
        setAllDataPoints([]);
        dataPointsRef.current = []; // Also clear the ref
      }

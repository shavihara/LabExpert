...
function ExperimentInterface() {
  ...
  // --- MODIFICATION: Replace floating messages state with a single status state ---
  ...
  // --- MODIFICATION: Update function to set a single status message ---
    ...
    const connectSSE = () => {
      ...
      eventSource.onerror = (err) => {
        ...
        if (isStreamConnected) {
          ...
          addStatusMessage('Data stream lost. Reconnecting...', 'warning');
        }
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Failed to start on ESP32');
      if (elapsedTime === 0) {
        handleReset();
      }
      setIsRunning(true);
      setIsPaused(false);
      setError(null);
      addStatusMessage('Experiment started', 'info');
    } catch (err) {
  ...
  const dataForTable = useMemo(() => (isAnalysisComplete ? analyzedData : allDataPoints).slice(showAllData ? 0 : -20).reverse(), [allDataPoints, analyzedData, isAnalysisComplete, showAllData]);
  ...
  // --- MODIFICATION: Create a Status Icon component ---
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50 to-slate-100 p-2 sm:p-4 pt-16 sm:pt-20">
      ...
      {/* --- MODIFICATION: Floating popup DIV is removed --- */}
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 mb-4 sm:mb-6">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-4 w-full sm:w-auto">
            ...
            {/* --- MODIFICATION: New dedicated status message component --- */}
            ...
            <div className="grid grid-cols-2 sm:flex gap-2 sm:gap-3">
              {!isRunning ? (
                <>
                  ...
                  <button onClick={handleStart} disabled={!esp32Connected} className="flex items-center justify-center gap-2 px-3 sm:px-6 py-2 sm:py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-xl font-bold hover:shadow-lg transition-all hover:-translate-y-1 disabled:opacity-50 text-xs sm:text-sm"><Play size={16} className="sm:w-5 sm:h-5" />Start</button>
                </>
              ) : (
            ...
            <div className={`${isFullscreen ? 'h-[calc(100vh-150px)]' : 'h-64 sm:h-96'} bg-gradient-to-br from-slate-50 to-purple-50 rounded-xl sm:rounded-2xl p-2 sm:p-4 border border-slate-200`}>
              {(allDataPoints.length > 0 || isAnalysisComplete) ? (
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={isAnalysisComplete ? analyzedData : allDataPoints} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                    ...
                    {comparisonMode ? (
                      ...
                      <>
                        ...
                        <Line 
                          type={getLineType(activeGraph)}
                          dataKey={activeGraph} 
                          stroke={getGraphColor(activeGraph)} 
                          strokeWidth={3} 
                          dot={false} 
                          name={isAnalysisComplete ? `Best-Fit ${activeGraph.charAt(0).toUpperCase() + activeGraph.slice(1)}` : activeGraph.charAt(0).toUpperCase() + activeGraph.slice(1)} 
